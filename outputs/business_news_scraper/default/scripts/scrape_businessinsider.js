...
(async () => {
    ...
    for (const selector of articleSelectors) {
      ...
      elements.forEach(el => {
        ...
        if (headline && headline.length > 10 && !seenHeadlines.has(headline) && url) {
          ...
          let dateText = '';
          let parent = el.closest('article') || el.closest('div[class*="tout"]') || el.closest('div[class*="story"]');
          if (parent) {
  ...
  fs.writeFileSync(
    '/Users/venkat/workfolder/playwright-min/outputs/business_news_scraper/default/raw/businessinsider.json',
